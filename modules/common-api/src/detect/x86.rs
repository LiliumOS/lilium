use core::{arch::x86_64::CpuidResult, cell::LazyCell};
...
pub static CPU_FEATURE_INFO: LazyCell<[u32; 48]> = LazyCell::new(init_cpuid_features);
...
#[macro_export]
#[doc(hidden)]
macro_rules! x86_feature_to_bit {
    ("x87") => {
        (1, 0)
    };
    ("cmpxchg8b") => {
...
macro_rules! is_x86_feature_detected {
    ($feature:tt) => {
        ::core::cfg!(target_feature = $feature)
            || ({
                let (idx, bit) = $crate::x86_feature_to_bit!($feature);
                ($crate::detect::x86::CPU_FEATURE_INFO[idx] & (1 << bit)) != 0
            })
    };
}
